...
type LeaseLister interface {
	...
	List(selector labels.Selector) (ret []*coordination.Lease, err error)
...
// List lists all Leases in the indexer.
func (s *leaseLister) List(selector labels.Selector) (ret []*coordination.Lease, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*coordination.Lease))
	})
	return ret, err
}
...
type LeaseNamespaceLister interface {
	...
	Get(name string) (*coordination.Lease, error)
...
// List lists all Leases in the indexer for a given namespace.
func (s leaseNamespaceLister) List(selector labels.Selector) (ret []*coordination.Lease, err error) {
...
// Get retrieves the Lease from the indexer for a given namespace and name.
func (s leaseNamespaceLister) Get(name string) (*coordination.Lease, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(coordination.Resource("lease"), name)
	}
	return obj.(*coordination.Lease), nil
}
